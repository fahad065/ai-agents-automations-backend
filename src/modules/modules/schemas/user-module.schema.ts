...
export class UserModule {
  ...
  // ── Country & Niche ───────────────────────────────────────
  @Prop({ default: 'UAE' })
  country: string;         // 'UAE' | 'Kenya' — set at subscribe time

  @Prop({ default: 'content_social' })
  nicheSlug: string;       // mirrors module.nicheSlug

  @Prop({ default: 'standalone' })
  pipelineCategory: string; // 'standalone' | 'niche_pipeline'

  @Prop({ type: [String], default: [] })
  enabledComponents: string[]; // keys of enabled components for niche pipelines
  ...
  @Prop({ default: 'auto' })
  videoModel: string;  // 'auto' | 'alibaba/wan-2.6/...' | 'bytedance/seedance-2.0-fast/...'
}
...
UserModuleSchema.index({ country: 1 });
UserModuleSchema.index({ nicheSlug: 1 });