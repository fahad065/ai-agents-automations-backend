  ...
  export class UserModulesController {
    ...
    subscribe(@Req() req: any, @Body() body: {
      ...
      return this.service.subscribe({
        ...
        pipelineCategory: body.pipelineCategory,
      });
    }
    ...
    // Fetch available Atlas video models with pricing
    @Get('atlas-models')
    async getAtlasModels(@Query('type') type?: string) {
      return this.service.getAtlasModels(type || 'youtube');
    }

    @Patch(':id/record-run')
    @Public()
    @UseGuards(PipelineSecretGuard)  // ← replaces JwtAuthGuard for this route
    async recordRun(
      @Param('id') id: string,
      @Body() body: { cost: number },
    ) {
      return this.service.recordRun(id, body.cost || 0);
    }
    ...
    updateSchedule(
      ...
      return this.service.update(id, req.user._id.toString(), {
